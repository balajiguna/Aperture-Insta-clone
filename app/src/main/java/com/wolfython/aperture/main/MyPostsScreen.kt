...
Column {
    Column(modifier = Modifier.weight(1f)) {
      ...
      PostList(isContextLoading = isLoading, postsLoading = postsLoading, posts =posts , modifier = Modifier.weight(1f).padding(1.dp).fillMaxSize()){
    ...
    if (postsLoading){
        ...
        LazyColumn(modifier = modifier){
            ...
            items(items = rows){ rows ->
                ...
                PostRow(item = rows, onPostClick = onPostClick)
